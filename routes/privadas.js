...
router.get("/admin/index", (peticion, respuesta) => {
  /*Renderiza la pagina de inicio del admin o empleado*/
  respuesta.render("admin/index", {
    usuario: peticion.session.usuario,
    /*Pasa el usuario que inicio sesion*/
    mensaje: peticion.flash(
      "mensaje"
    ) /*Muestra mensaje si se redirigio a este endpoint y muestra el mensaje que viene acompañado con la redireccion*/ ,
  })
})
...
router.get("/admin/registrar_entrada", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    ...
    connection.query(consulta, (error, filas, campos) => {
      ...
      respuesta.render("admin/registrar_entrada", {
        /*Renderiza la pagina web con los datos obtenidos*/
        usuario: peticion.session.usuario,
        /*Pasa el usuario que inicio sesion*/
        mensaje: peticion.flash("mensaje"),
        /*Muestra mensaje si se redirigio de nuevo a este endpoint y muestra el mensaje que viene acompañado con la redireccion*/
        vehiculos: filas,
        /*Pasa los datos obtenidos como una lista al html*/
      })
    })
...
router.post("/admin/procesar_entrada", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    ...
    const tipo_vehiculo = parseInt(
      peticion.body.t_veh
    ) /*Se convierte a entero el valor que pasa el select en el formulario HTLM*/
    const placa = peticion.body.placa
      .toLowerCase()
      .trim() /*Se pasa a minuscula y se quitan espacios de la placa*/
    const consulta = `INSERT INTO reg_vehiculos (placa,hora_entrada, estado, id_empleado,tipo_vec) VALUES (${connection.escape(
      placa
    )},${connection.escape(fecha)},"IN", ${connection.escape(
      peticion.session.usuario.id_empleado
    )},${connection.escape(tipo_vehiculo)})
    ` /* Se crea la consulta para insertar los datos correspondientes en la BD*/
    connection.query(consulta, (error, filas, campos) => {
      peticion.flash("mensaje", "Entrada Registrada a las " + tiempo_entrada) /*Se crea un mensaje flash para mostrar luego de realizar el registro*/
...
router.get("/admin/registrar_salida", (peticion, respuesta) => {
  respuesta.render("admin/registrar_salida", {
    /*Renderiza el archivo ejs correspondiente como un html*/
    usuario: peticion.session.usuario,
    /*Pasa el usuario que inicio sesion*/
    mensaje: peticion.flash("mensaje"),
    /*Muestra mensaje si se redirigio a este endpoint y muestra el mensaje que viene acompañado con la redireccion*/
...
router.post("/admin/procesar_salida", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    ...
    const consulta_hora_entrada = `select hora_entrada, hora_salida, cobroxmin, ntipo_veh from reg_vehiculos, tipo_veh WHERE placa =${connection.escape(
      placa
    )} and reg_vehiculos.tipo_vec=tipo_veh.id_tipo` /*Se crea la consulta la hora de entrada del vehiculo de la placa dada*/
    connection.query(consulta_hora_entrada, (error, filas, campos) => {
      /*Realiza la consulta y los datos obtenidos los inserta en la variable "filas"*/
      if (filas.length > 0 && filas[0].hora_salida == null) {
        const hora_entrada = new Date(filas[0].hora_entrada)
        /*const hora_entrada_format = `${hora_entrada.getFullYear()}-${hora_entrada.getMonth() + 1}-${hora_entrada.getDate()} ${hora_entrada.getHours()}:${hora_entrada.getMinutes()}:${hora_entrada.getSeconds()}`*/
        ...
        const consulta_Update = `UPDATE reg_vehiculos SET hora_salida = ${connection.escape(
          hora_salida
        )}, total_pago = ${connection.escape(
          cobro
        )}, estado = "OUT", total_tiempo_esta = ${connection.escape(
          tiempo
        ...
        connection.query(consulta_Update, (error, filas, campos) => {
          /*Se Realiza la consulta UPDATE*/
          peticion.flash(
            "mensaje",
            "El pago es " +
            cobro +
...
router.get("/admin/reportes", (peticion, respuesta) => {
  respuesta.render("admin/reportes", {
    usuario: peticion.session.usuario,
    mensaje: peticion.flash("mensaje"),
  }) /*Endpoint de los reportes, se renderiza el archivo ejs correspondiente*/
})
...
router.get("/admin/reporte1", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    ...
    if (fecha1 != "" || fecha2 != "") { /*Si las fechas o al menos una no es vacia entra en el if*/
      ...
      const consulta = `
      SELECT SUM(total_pago) AS Total_Recaudado, COUNT(id_vehiculo) AS Total_Vehiculos FROM reg_vehiculos WHERE hora_salida LIKE '%${fecha1}%' OR hora_salida BETWEEN '${fecha1}' AND '${fecha2}'`
      /*Se crea la consulta necesaria*/
      connection.query(consulta, (error, filas, campos) => { /*Realiza la consulta y los datos obtenidos los inserta en la variable "filas"*/
        respuesta.render("admin/reporte1", { /*Renderiza el archivo .ejs correspondiente*/
          usuario: peticion.session.usuario, /*Pasa el usuario que inicio sesion*/
          total_rec: String(filas[0].Total_Recaudado),/*Se extraen los datos obtenidos y se pasan como tipo String, total recaudado*/
          total_veh: String(filas[0].Total_Vehiculos),/*total vehiculos*/
          fecha1: fecha1, /*Se pasan als fechas */
          fecha2: fecha2,
        })
      })
    } else {
      respuesta.render("admin/reporte1", { /*Aqui se renderiza la pagina correspondiete sin ningun tipo de consulta, no muestra datos*/
        usuario: peticion.session.usuario,
        total_rec: "",
        total_veh: "",
        fecha1: fecha1,
...
router.get("/admin/reporte2", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    ...
    if (fecha1 != "" || fecha2 != "") { /*Si las fechas o al menos una no es vacia entra en el if*/
      ...
      fecha2 = `${fecha2.getFullYear()}-${
        fecha2.getMonth() + 1
        }-${fecha2.getDate()} ${fecha2.getHours()}:${fecha2.getMinutes()}:${fecha2.getSeconds()}`
      /*A esta fecha se le suma un dia para hacer la consulta hasta el final del dia que se quiere*/
      const consulta = `
      SELECT id_vehiculo, placa, hora_entrada, hora_salida, total_tiempo_esta, total_pago,ntipo_veh
      FROM reg_vehiculos,tipo_veh
      WHERE reg_vehiculos.tipo_vec=tipo_veh.id_tipo AND (hora_salida LIKE '%${fecha1}%' OR hora_salida BETWEEN '${fecha1}' AND '${fecha2}') AND reg_vehiculos.estado="OUT"`
      /*Se crea la consulta necesaria*/
      connection.query(consulta, (error, filas, campos) => { /*Se realiza la consulta*/
        respuesta.render("admin/reporte2", { /*Se renderiza el archivo .ejs correspondiente y se le pasan los datos necesarios*/
          usuario: peticion.session.usuario,
          registros: filas,
          fecha1: fecha1,
          fecha2: fecha2,
...
router.get("/admin/reporte3generado", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    ...
    const tipo_vehiculo = parseInt(peticion.query.t_veh)
    ...
    const consulta = `
      SELECT id_vehiculo, placa, hora_entrada, hora_salida, total_tiempo_esta, total_pago,ntipo_veh
      FROM reg_vehiculos,tipo_veh
      WHERE reg_vehiculos.tipo_vec=tipo_veh.id_tipo AND tipo_veh.id_tipo=${tipo_vehiculo} AND (hora_salida LIKE '%${fecha1}%' OR hora_salida BETWEEN '${fecha1}' AND '${fecha2}') AND reg_vehiculos.estado="OUT"`
    /*Se crea la consulta necesaria*/
    connection.query(consulta, (error, filas, campos) => { /*Realiza la consulta y los datos obtenidos los inserta en la variable "filas"*/
...
router.post("/admin/procesar_new_veh", (peticion, respuesta) => {
  pool.getConnection((err, connection) => { /*Se crea la conexión con la base de datos*/
    var newVeh =
      ...
      peticion.body.new_veh.substr(1).toLowerCase() 
    /*Al tipo de auto nuevo se vuelve todo minuscula y la primera letra mayusacula*/
    const consultaVeh = `
          SELECT *
          FROM tipo_veh
